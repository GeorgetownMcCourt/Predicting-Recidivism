...
# confusion matrix
c.matrix.knn <- table(full.opt$CH_CRIMHIST_COLLAPSED, pred.full.opt) # pretty good ROC
c.matrix.knn
...
# ggplot(data=roc.knn, aes(x=true, y=predicted)) + 
#   geom_violin(fill=rgb(1,1,1,alpha=0.6), color=NA) + 
#   geom_jitter(aes(color=ROC), alpha=0.6) +
#   scale_color_discrete(name = "type")
#   --> this is really useful for GLM predictions, not predictions for predicted class rather than likelihoods
